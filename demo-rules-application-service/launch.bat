...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "demo-rules-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name demo-rules-application-service apps/demo-rules-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app demo-rules-application-service:1.0-SNAPSHOT
		call oc expose svc/demo-rules-application-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd demo-rules-application-service
		cd target
