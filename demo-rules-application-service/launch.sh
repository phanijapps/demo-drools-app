...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "demo-rules-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name demo-rules-application-service apps/demo-rules-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app demo-rules-application-service:1.0-SNAPSHOT
    oc expose svc/demo-rules-application-service
else

	echo "Launching the application locally..."
	pattern="demo-rules-application-service"
	files=( $pattern )
	cd ${files[0]}
